...
class Constraints {
  ...
  static relationshipToTile(
    location: Position,
    args: ConstraintArguments
  ): TileRelation | undefined {
    if (!location) {
      return undefined;
    }
    const { piece } = args;
...
class Board {
  ...
  initializeState() {
    for (let pieceId in this.pieces) {
      const { row, col, active, _moves, _promoted } =
  ...
  kingCastles(king: Piece): Position[] {
    ...
    const checkPositions = (
      row: PositionRow,
      rookCol: PositionColumn,
      castles: Position[]
    ) => {
      const cols: PositionColumn[] =
        rookCol === "A" ? ["D", "C", "B"] : ["F", "G"];
...
class Control {
  game: Game;
  inputPerspectiveBlack: HTMLInputElement = document.getElementById(
    "black-perspective"
  ) as HTMLInputElement;
  inputPerspectiveWhite: HTMLInputElement = document.getElementById(
    "white-perspective"
  ) as HTMLInputElement;
  view: View;
